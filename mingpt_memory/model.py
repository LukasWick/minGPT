...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_kc = nn.Linear(self.n_kc, config.n_embd) # knowledge contet
    ...
    def forward(self, x):
        ...
        z = torch.ones((B,self.n_kc,C)) #Knowledge content vectors
        z = self.c_kc(z)
        ...
        x.append(z,dim=2)
        k,v = self.c_attn_q(x).split(self.n_embd, dim=2)

        k = k.view(B, T+self.n_kc, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        q = q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = v.view(B, T+self.n_kc, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
