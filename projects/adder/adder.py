...
class AdditionDataset(Dataset):
    ...
    def __init__(self, config, split):
        ...
        num_test = min(int(num*self.config.throwaway), 1000) # 20% of the whole dataset, or only up to 500
        
        # Trow away some samples to create a gap between train and test sets to test generalization better
        num_throwaway = int(num*self.config.throwaway)-num_test
        
        self.ixes = perm[:num_test] if split == 'test' else perm[num_test+num_throwaway:]
    ...
    def __getitem__(self, idx):
        ...
        # In 1% of cases, substitute one character with another random digit
        if self.split == 'train' and torch.rand(1).item() < self.config.fuzzy_prob:
            cstr_list = list(cstr)
            pos = torch.randint(0, len(cstr_list), (1,)).item()
            original_digit = int(cstr_list[pos])
            new_digit = torch.randint(0, 10, (1,)).item()
            # Ensure we pick a different digit
            while new_digit == original_digit:
                new_digit = torch.randint(0, 10, (1,)).item()
            cstr_list[pos] = str(new_digit)
            cstr = ''.join(cstr_list)
...
if __name__ == '__main__':
    ...
    def batch_end_callback(trainer):
        ...
        if trainer.iter_num % (10**int(torch.log10(torch.tensor(trainer.iter_num+1, dtype=torch.float)).item())) == 0:
